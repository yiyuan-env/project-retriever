...
async function robustInspect() {
    ...
    console.log('--- Clicking first tender and handling new tab ---');
    // PCC often opens in new tab or redirects
    ...
    if (newPage) {
        ...
        const pccData = await newPage.evaluate(() => {
            const results = {};
        ...
        console.log('PCC Detail Data (Tab):', JSON.stringify(pccData, null, 2));
    } else {
        console.log('No new tab detected, checking current page...');
        const pccData = await page.evaluate(() => {
            const results = {};
            const rows = Array.from(document.querySelectorAll('tr'));
            rows.forEach(tr => {
                const th = tr.querySelector('th');
        ...
        console.log('PCC Detail Data (Page):', JSON.stringify(pccData, null, 2));
    }

    console.log('\n--- Checking TaiwanBuying Detail ---');
    // Let's go to a known detail page directly since TB WAF is less aggressive
